package com.example.souqadmin.ui.soldOut.adapter
...
class SoldOutAdapter (val context : Context) :
    ...
    fun changeData(newData: ArrayList<ProductResponse.Item>,clearOldData:Boolean=false) {
        ...
        val diffResult: DiffUtil.DiffResult = DiffUtil.calculateDiff(
            SoldOutAdapter.ItemsDiffCallback(
                oldData,
                newData
            )
    ...
    inner class ViewHolder(val binding: ProductItemRvBinding) :
        ...
        fun binding(item: ProductResponse.Item, position: Int) = with(itemView) {
            binding.deleteBtn.setImageResource(R.drawable.ic_sold_out)
            Glide.with(context)
                .load(item.image)
                .fitCenter()
